...
**v4.0**: モバイルはボトムタブのアプリシェル（今日/ランキング/ウォッチ/分析）＋PWA対応
（スマホで「ホーム画面に追加」するとアプリとして起動）。詳細シートは左右スワイプで前後銘柄。
ユニバースは「継続選出コア（過去10日4回以上・自己参照ルール）＋スクリーナー新規」のハイブリッドで、
day_gainers系の日次入替バイアス（実測~28%/日）を緩和。
